...
class RepConv(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        map_k = kernel_size if map_k is None else map_k
        assert map_k <= kernel_size
        self.origin_kernel_shape = (out_channels, in_channels // groups,
                                    kernel_size, kernel_size)
        self.register_buffer('_weight', torch.zeros(*self.origin_kernel_shape))
    ...
    @property
    def weight(self):
        origin_weight = self._weight.view(1, self.num_2d_kernels,
                                          self.kernel_size, self.kernel_size)
        return self._weight + self.convmap(origin_weight).view(
            *self.origin_kernel_shape)
...
class RepVGGUnet(BaseModule):
    ...
    def __init__(self,
                 ...
                 out_channels: int = 3,
                 base_width: int = 16,
                 use_rep: bool = False) -> None:
        super().__init__()
        self.encoder = UnetEncoder(in_channels, base_width, use_rep=use_rep)
        self.decoder = UnetDecoder(
            base_width, out_channels=out_channels, use_rep=use_rep)
...
if __name__ == '__main__':
    net = RepVGGUnet(in_channels=3, out_channels=3, base_width=16)
    net.eval()
