...
class SEBlock(nn.Module):
    ...
    def __init__(self,
                 input_channels: int,
                 internal_neurons: Optional[int] = None):
...
class DFL(nn.Module):
    ...
    def reset_dfl(self):
        # x = torch.linspace(-1, 1, self.reg_max)
        x = torch.linspace(-0.35, 0.75, self.reg_max)
...
class RepVGGUnetFinal(BaseModule):
...
if __name__ == '__main__':
    net = RepVGGUnetFinal(
        in_channels=3, out_channels=3, base_width=16, reg_max=7)
    net.eval()
