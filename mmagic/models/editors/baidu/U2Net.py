...
class RSU(nn.Module):
    ...
    def __init__(self, height: int, in_ch: int, mid_ch: int, out_ch: int):
        ...
        for i in range(height - 2):
            ...
            decode_list.append(
                UpConvBNReLU(mid_ch * 2, mid_ch if i < height - 3 else out_ch))
...
class U2NetBaidu(BaseModule):
    ...
    def __init__(self, cfg: dict, out_ch: int = 3):
        ...
        for c in cfg['encode']:
            ...
            assert len(c) == 6
            encode_list.append(
                RSU(*c[:4]) if c[4] is False else RSU4F(*c[1:4]))
    ...
    def forward(self,
        ...
        for m in self.side_modules:
            ...
            x = F.interpolate(
                m(x), size=[h, w], mode='bilinear', align_corners=False)
...
def convert_onnx(m, save_path):
    m.eval()
    x = torch.rand(1, 3, 1024, 1024, requires_grad=True)
...
if __name__ == '__main__':
    ...
    u2net = u2net_lite(out_ch=3)
    param_mb = sum(m.numel() * m.element_size()
                   for m in u2net.parameters()) / (1 << 20)
    print(f'Model size: {param_mb:.2f} MB')
    convert_onnx(u2net, 'u2net_lite.onnx')
